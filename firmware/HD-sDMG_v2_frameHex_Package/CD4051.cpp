...
void CD4051_Manager::Loop(void)
{
  ...
  if(this->_group > this->_groupNum - 1)   
  {
    ...
    // this->_frameNum++;
    // Serial.println(this->_frameNum);

    // Serial1.print("@$%");
    this->_sendPackage[this->_sendPackageIndex++] = '@';
    this->_sendPackage[this->_sendPackageIndex++] = '$';
    this->_sendPackage[this->_sendPackageIndex++] = '%';
    for(i = 0; i < this->_chipNum; i++)
    {
      lineNum = this->_chip[i].GetLineNum();
      for(j = 1; j <= lineNum; j++)   //group-line
      {
        temp = this->_chip[i].GetData(j, this->_index[j - 1]);
        this->_sendPackage[this->_sendPackageIndex++] = (temp >> 8);
        this->_sendPackage[this->_sendPackageIndex++] = temp;
        // Serial1.write(temp >> 8);
        // Serial1.write(temp);
      }
    }
    this->_sendPackage[this->_sendPackageIndex++] = '&';
    this->_sendPackage[this->_sendPackageIndex++] = '\r';
    this->_sendPackage[this->_sendPackageIndex++] = '\n';
    // Serial1.print("&\r\n");

    // 缓存存满，可以发送
    if(this->_sendPackageIndex >= this->_packageLen)
    {
      this->_frameNum++;
      //Serial.println(this->_frameNum);
      
      this->_sendPackageIndex = 0;
      for(i = 0; i < this->_packageLen; i++)
      {
        Serial1.write(this->_sendPackage[i]);
      }

      while(micros() < this->_remember);
      this->_remember = this->_packageTc + micros();
      // Serial.println(micros() - this->_remember);
    }
  }
...
//设置采样频率，1~130，单位Hz
void CD4051_Manager::setFreq(uint8_t freq)
{
  if(freq < 1)  
    this->_freq = 1;
  else if(freq > 130)  
  ...
  this->_Tc = 1000000 / this->_freq;
  this->_packageTc = PACKAGE_FRAMENUM * this->_Tc;

  OLED_ShowNum(30,24,this->_freq,3,8);
  OLED_Refresh();  
}
